...
export default function FlameStory() {
  ...
  useGSAP(() => {
    ...
    if (isDesktop) {
      // The kitchen image stays fixed while the ritual steps scrub into focus.
      ...
      stepEls.forEach((step) => {
        gsap.fromTo(step, { autoAlpha: 0.2, y: 32 }, {
          autoAlpha: 1, y: 0, ease: "none",
          scrollTrigger: { trigger: step, start: "top 85%", end: "top 55%", scrub: true },
        });
      });
    } else {
      // Mobile: no pin — a gentle one-shot reveal instead.
      ...
      gsap.from(stepEls, {
        autoAlpha: 0, y: 24, duration: 0.6, stagger: 0.12, ease: "power2.out",
        clearProps: "transform,opacity,visibility",
  ...
  return <section ref={sectionRef} id="flame-story" className="overflow-hidden bg-navy px-5 py-20 text-white md:px-10 md:py-28"><div className="mx-auto grid max-w-[1360px] items-center gap-12 md:grid-cols-2 md:gap-16"><div ref={mediaRef}><article className="group rounded-[28px] bg-white p-3 pb-7 text-ink shadow-2xl shadow-black/10 transition-transform duration-500 hover:-translate-y-2 md:p-4 md:pb-8"><div className="relative aspect-[4/3] overflow-hidden rounded-[21px] bg-marble"><Image src="/images/carving-gyro.png" alt="Greek Mansion cook carving chicken gyro from the vertical spit" fill className="object-cover object-[55%_42%] transition-transform duration-700 ease-out group-hover:scale-[1.04]" sizes="(max-width:768px) 100vw,50vw"/></div><div className="px-2 pt-6"><p className="font-label text-[9px] uppercase tracking-[.2em] text-gold">Open flame · Every day</p><p className="mt-2 font-serif text-3xl uppercase leading-none text-navy md:text-4xl">The heart of our kitchen</p></div></article></div><div className="flex items-center md:px-6 lg:px-12"><div ref={copyRef}><SectionTitle eyebrow="The ritual" title="From the flame" light/><p className="mt-7 max-w-lg text-sm leading-7 text-white/65">A simple process, repeated with patience. The flame does not hurry—and neither do we.</p><ol className="mt-12 space-y-8">{steps.map(([Icon, t, d], i) => {const C = Icon as typeof Leaf;return <li className="flame-step grid grid-cols-[40px_1fr] gap-5 border-b border-white/15 pb-7" key={t}><C className="text-gold" size={23}/><div><p className="font-serif text-2xl">0{i + 1}. {t}</p><p className="mt-1 text-xs text-white/50">{d}</p></div></li>;})}</ol></div></div></div></section>;
}
