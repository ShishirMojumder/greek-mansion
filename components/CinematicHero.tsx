...
export default function CinematicHero() {
  return <section className="cinematic-hero noise relative min-h-[calc(100svh-80px)] overflow-hidden bg-ink">
    <motion.div className="absolute inset-0" initial={{scale:1.035,opacity:0}} animate={{scale:1,opacity:1}} transition={{duration:1.5,ease}}><Image src="/images/hero-greek-wraps.png" alt="Greek chicken wraps, fries, salad and mezze arranged on a sunlit table" fill priority className="object-cover object-[62%_50%] md:object-center" sizes="100vw"/></motion.div>
    ...
    <div className="relative z-20 mx-auto flex min-h-[calc(100svh-80px)] max-w-[1400px] flex-col items-center justify-center px-5 pb-12 pt-24 text-center md:items-start md:px-10 md:pb-10 md:pt-24 md:text-left lg:px-16">
      ...
      <motion.h1 className="display max-w-[790px] text-[clamp(4.1rem,9vw,8.6rem)] uppercase text-white" initial={{ opacity: 0, y: 55 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .3, duration: 1.05, ease }}>Authentic<br/><span className="text-gold">Greek Cuisine</span></motion.h1>
      <motion.p className="mt-5 max-w-lg text-sm leading-6 text-white/75 md:text-base md:leading-7" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .58, duration: .75, ease }}>Traditional flavours. Modern hospitality.<br/>A taste of Greece in Scarborough.</motion.p>
