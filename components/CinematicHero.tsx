...
export default function CinematicHero() {
  ...
  return <section ref={sectionRef} className="cinematic-hero noise relative min-h-[calc(100svh-80px)] overflow-hidden bg-ink">
    <motion.div ref={bgRef} className="absolute inset-0" initial={{scale:1.035,opacity:0}} animate={{scale:1,opacity:1}} transition={{duration:1.5,ease}}><Image src="/images/real-food/hero-counter.jpg" alt="The Greek Mansion counter in Scarborough, menu boards lit above the service line" fill priority className="object-cover object-center" sizes="100vw"/></motion.div>
    <div className="absolute inset-0 bg-ink/20"/>
    <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(17,25,54,.70)_0%,rgba(30,42,120,.66)_45%,rgba(17,25,54,.86)_100%)] max-md:bg-[linear-gradient(180deg,rgba(17,25,54,.82),rgba(17,25,54,.90))]"/>
    <div className="absolute inset-0 bg-[radial-gradient(ellipse_62%_54%_at_50%_46%,rgba(17,25,54,.60)_0%,rgba(17,25,54,.34)_55%,rgba(17,25,54,0)_100%)]"/>
    ...
    <div ref={contentRef} className="relative z-20 mx-auto flex min-h-[calc(100svh-80px)] max-w-[1400px] flex-col items-center justify-center px-5 pb-12 pt-24 text-center md:px-10 md:pb-10 md:pt-24 lg:px-16">
      ...
      <motion.h1 className="display max-w-[790px] text-[clamp(4.1rem,9vw,8.6rem)] uppercase text-white" initial={{ opacity: 0, y: 55 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .3, duration: 1.05, ease }}>Fresh <span className="accent">Greek</span><br/><span className="text-gold">Fair <span className="accent">Prices</span></span></motion.h1>
      <motion.p className="mt-5 max-w-xl text-sm leading-6 text-white/75 md:text-base md:leading-7" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .58, duration: .75, ease }}>Flame-grilled souvlaki, gyros and mezze—made fresh every day.<br/>Generous Greek plates, priced for the whole table.</motion.p>
      <motion.div className="mt-6 flex flex-wrap justify-center gap-3 md:mt-7" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .72, duration: .75, ease }}><MagneticLink href="/menu" primary>View menu</MagneticLink><MagneticLink href="https://www.ubereats.com/ca/store/greek-mansion/2jMowxE1Ts6w_sOBYT_BQA?utm=greekfooddelivery.ca">Order online</MagneticLink></motion.div>
