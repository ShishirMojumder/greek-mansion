...
export default function GreekMansionFooter(){return <footer className="marble w-full px-4 py-8 md:px-8 md:py-12"><motion.div className="w-full" initial="hidden" whileInView="visible" viewport={{once:true,margin:"-80px"}} variants={container}>
 <div className="flex flex-col gap-4 md:flex-row">
  <motion.div variants={item} className="footer-brand-card noise relative flex min-h-[430px] w-full flex-col justify-between overflow-hidden rounded-2xl bg-navy p-7 text-white md:min-h-[610px] md:w-[35%] md:p-10">
   ...
   <div className="relative z-10"><p className="eyebrow eyebrow-gold">Greek Kitchen · Scarborough</p><h2 className="display mt-5 text-5xl uppercase md:text-6xl">Come <span className="accent">hungry</span>.</h2><div className="mt-2"><HandwritingText text="Leave as family" height="3.25rem" className="text-gold" duration={1.7}/></div><p className="mt-6 max-w-xs text-sm leading-7 text-white/60">Family recipes, open-flame cooking and warm Greek hospitality in the heart of Scarborough.</p>
    ...
    <div className="mt-8 flex flex-wrap gap-2"><a href="tel:+14162923333" aria-label="Call Greek Mansion" className="footer-action"><Phone size={16}/>Call</a><a href="https://maps.app.goo.gl/bRD3NCp2ohiZZSp19" target="_blank" rel="noreferrer" aria-label="Directions to Greek Mansion" className="footer-action"><MapPin size={16}/>Directions</a><a href="https://www.ubereats.com/ca/store/greek-mansion/2jMowxE1Ts6w_sOBYT_BQA?utm=greekfooddelivery.ca" target="_blank" rel="noreferrer" aria-label="Order Greek Mansion on Uber Eats" className="footer-action"><ShoppingBag size={16}/>Order</a><a href="https://www.facebook.com/profile.php?id=61593538364319" target="_blank" rel="noreferrer" title="Greek Mansion on Facebook" aria-label="Visit Greek Mansion on Facebook" className="footer-action footer-action-icon"><Facebook size={16}/></a><a href="https://www.instagram.com/greekmansion/" target="_blank" rel="noreferrer" title="Greek Mansion on Instagram" aria-label="Visit Greek Mansion on Instagram" className="footer-action footer-action-icon"><Instagram size={16}/></a></div>
   </div>
  </motion.div>
